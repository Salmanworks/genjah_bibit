...
<section class="relative overflow-hidden" style="background: #2B3A28;">
    ...
    {{-- Dot grid --}}
    <div class="absolute inset-0 pointer-events-none" style="background-image: radial-gradient(circle, rgba(61, 92, 56, 0.05) 1px, transparent 1px); background-size: 24px 24px;"></div>
    ...
    <div class="relative max-w-7xl mx-auto px-6 sm:px-8 lg:px-12" style="padding-top: 56px; padding-bottom: 72px;">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div style="padding:28px 24px; background:rgba(255,255,255,0.05); border:1px solid rgba(197, 232, 122, 0.2); border-radius:12px; position:relative; overflow:hidden; transition:all 0.3s ease;" onmouseover="this.style.background='rgba(255,255,255,0.08)'; this.style.transform='translateY(-4px)';" onmouseout="this.style.background='rgba(255,255,255,0.05)'; this.style.transform='translateY(0)';">
                {{-- Number watermark --}}
                <div style="position:absolute; top:-10px; right:12px; font-size:5rem; font-weight:900; color:rgba(197, 232, 122, 0.08); line-height:1; user-select:none;">{{ str_pad($loop->index + 1, 2, '0', STR_PAD_LEFT) }}</div>
                {{-- Icon --}}
                <div style="width:44px; height:44px; border-radius:8px; background:rgba(197, 232, 122, 0.15); border:1px solid rgba(197, 232, 122, 0.3); display:flex; align-items:center; justify-content:center; margin-bottom:20px; color:#c5e87a;">
                    {!! $feature['icon'] !!}
                </div>
                {{-- Divider --}}
                <div style="width:32px; height:2px; background:#c5e87a; border-radius:2px; margin-bottom:14px; opacity:0.7;"></div>
