...
<section class="relative mb-8">
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 class="text-3xl font-extrabold text-white tracking-tight">Edit Blog</h1>
            <p class="text-emerald-100/40 text-sm">Edit: {{ $blog->title }}</p>
        </div>
    </div>
</section>
    ...
    <form action="{{ route('admin.blogs.update', $blog) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="admin-card p-8 rounded-2xl space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $blog->title) }}" required placeholder="Masukkan judul blog..." class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/30 focus:outline-none focus:border-lime-500 focus:bg-white/10 transition-all @error('title') border-red-500 @enderror">
                @error('title')
            ...
            <div>
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        @if($blog->featured_image)
                            <p class="text-xs text-lime-400 mt-1">Gambar saat ini: {{ basename($blog->featured_image) }}</p>
                        @endif
            ...
            <div>
                ...
                <input type="datetime-local" name="published_at" value="{{ old('published_at', $blog->published_at ? $blog->published_at->format('Y-m-d\TH:i') : '') }}" class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:border-lime-500 focus:bg-white/10 transition-all @error('published_at') border-red-500 @enderror">
            ...
            <div class="space-y-3">
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" name="is_published" value="1" {{ old('is_published', $blog->is_published) ? 'checked' : '' }} class="w-5 h-5 rounded bg-white/5 border-2 border-white/20 text-lime-500 focus:ring-2 focus:ring-lime-500 focus:ring-offset-0 transition-all">
                    <span class="text-sm font-bold text-white group-hover:text-lime-400 transition-colors">Publish blog ini</span>
                </label>
                
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $blog->is_featured) ? 'checked' : '' }} class="w-5 h-5 rounded bg-white/5 border-2 border-white/20 text-lime-500 focus:ring-2 focus:ring-lime-500 focus:ring-offset-0 transition-all">
            ...
            <div class="pt-6 border-t border-white/10">
                <div class="grid grid-cols-3 gap-4">
                    <div class="text-center">
                        <p class="text-2xl font-bold text-lime-400">{{ $blog->view_count ?? 0 }}</p>
                        <p class="text-xs text-white/40">Total Views</p>
                    </div>
                    <div class="text-center">
                        <p class="text-2xl font-bold text-lime-400">{{ $blog->reading_time ?? 1 }}</p>
                        <p class="text-xs text-white/40">Min Read</p>
                    </div>
                    <div class="text-center">
                        <p class="text-2xl font-bold text-lime-400">{{ $blog->created_at->format('d M Y') }}</p>
                        <p class="text-xs text-white/40">Dibuat</p>
                    </div>
                </div>
            </div>
        </div>
