...
<body class="min-h-screen admin-body">
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-72 admin-sidebar flex flex-col transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out border-r border-white/5">
        ...
        <nav class="p-4 space-y-2 flex-1 overflow-y-auto" style="max-height: calc(100vh - 180px); scrollbar-width: thin; scrollbar-color: rgba(255, 255, 255, 0.3) transparent;">
            ...
            <!-- Edit Tentang Kami -->
            <a href="{{ route('admin.settings.about') }}" class="sidebar-link flex items-center gap-4 px-5 py-3 rounded-2xl {{ request()->routeIs('admin.settings.about') ? 'active' : 'text-lime-100/60 hover:bg-white/5 hover:text-white' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                <span class="font-semibold text-base">Tentang Kami</span>
            </a>
            
            <a href="{{ route('admin.settings.index') }}" class="sidebar-link flex items-center gap-4 px-5 py-3 rounded-2xl {{ request()->routeIs('admin.settings.index') ? 'active' : 'text-lime-100/60 hover:bg-white/5 hover:text-white' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
